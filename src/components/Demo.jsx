...
const Demo = () => {
  ...
  useEffect(() => {
    const articlesFromLocalStorage = JSON.parse(localStorage.getItem(LOCALSTORAGE_ARTICLES_KEY))
  ...
  const handleSubmit = async e => {
    ...
    localStorage.setItem(LOCALSTORAGE_ARTICLES_KEY, JSON.stringify(updatedAllArticles))
  ...
  return (
    <section className="mt-16 w-full max-w-xl">
      ...
      <div className="flex flex-col w-full gap-2">
        ...
        >
          ...
          <input
            ...
            className="url_input peer placeholder:text-gray-900 border-[#000] border-2 border-solid"
            required
          />
        ...
        {!isFetching && (
          <div className="flex flex-col gap-1 max-h-60 overflow-y-auto">
            ...
            {allArticles.map((article, i) => (
              <div className="link_card hover:bg-gray-950 " key={`link-${i}`}>
                <button type="button" className="copy_btn" onClick={() => handleCopy(article.url)}>
                  {/* copy to clipboard */}
                  <img
                    src={copied === article.url ? tick : copy}
                    alt={copied === article.url ? 'Copied' : 'Copy to Clipboard'}
                    title={copied === article.url ? 'Copied' : 'Copy to Clipboard'}
                ...
                {/* article url */}
                <button
                  className="hover:underline flex flex-1 font-satoshi text-blue-500 hover:text-white font-medium text-sm truncate cursor-pointer"
      ...
      <div className="my-10 max-w-full flex justify-center items-center">
        {isFetching ? (
          // loader
          <img src={loader} alt="Loading..." className="w-20 h-20 object-contain" loading="lazy" />
        ) : error ? (
          // error
